...
void CPrincess::putRoom()
{
	...
	//getimage(pimg_star_player, "PNG", MAKEINTRESOURCEA(STAR_PLAYER));
	PIMAGE pimg_star_end = newimage();
	//getimage(pimg_star_end, "PNG", MAKEINTRESOURCEA(STAR_END));
	...
	for (; is_run(); delay_fps(60))
	{
		...
		for (; is_run(); delay_fps(60))
		{
			...
			if (button->ifClick(xClick, yClick, 513, 400, 603, 430))
			{
				...
				wchar_t* text[10];
				text[0] = L"确定回到主页？\n";
				//button->putButton(513, 400, "回到主页");
				//button->putButton(513, 350, "暂停");
